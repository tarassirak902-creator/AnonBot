...
async def try_match_user(user_id: int) -> int | None:
    ...
    Recent partners are deprioritized for 30 minutes. When privacy-safe quality
    aggregates exist, candidate quality is used only as a soft ordering signal;
    FIFO remains the tie-breaker and new users stay neutral.
    """
    async with aiosqlite.connect(DB_PATH, timeout=10) as conn:
        ...
        try:
            ...
            has_quality = await _table_exists(conn, "match_quality")
            requester_score = 50
            if has_quality:
                quality_row = await (
                    await conn.execute(
                        "SELECT score FROM match_quality WHERE user_id=?",
                        (user_id,),
                    )
                ).fetchone()
                if quality_row:
                    requester_score = int(quality_row[0] or 50)

            if has_recent_partners and has_quality:
                candidate_sql = """SELECT q.user_id
                       FROM queues q
                       JOIN users u ON u.user_id=q.user_id
                  LEFT JOIN match_quality mq ON mq.user_id=q.user_id
                      WHERE q.user_id!=?
                      ...
                      ORDER BY
                        ...
                        CASE WHEN COALESCE(mq.ratings,0)>=3 AND COALESCE(mq.score,50)<30 THEN 1 ELSE 0 END,
                        ABS(COALESCE(mq.score,50)-?),
                        q.created_at ASC,
                        q.rowid ASC
                      LIMIT 1"""
                candidate_params = (user_id, user_id, requester_score)
            elif has_recent_partners:
                candidate_sql = """SELECT q.user_id
                       FROM queues q
                       JOIN users u ON u.user_id=q.user_id
                      WHERE q.user_id!=?
                ...
                candidate_sql = """SELECT q.user_id
                  ...
                  LEFT JOIN match_quality mq ON mq.user_id=q.user_id
                      ...
                      ORDER BY
                        ...
                        q.rowid ASC
                      LIMIT 1"""
                candidate_params = (user_id, requester_score)
            else:
                candidate_sql = """SELECT q.user_id
                       FROM queues q
                       JOIN users u ON u.user_id=q.user_id
