...
async def successful_question_premium_payment(message: Message):
    ...
    if not receiver_id or allowed.get(months) != stars or int(payment.total_amount) != stars:
        ...
        return payment_reconciliation_required("Premium receiver, duration or amount became invalid after payment")
    ...
    if status in {"delivered", "failed", "delivering"}:
        ...
        return payment_reconciliation_required(f"Premium delivery is in {status} state")

    if not await db.claim_premium_delivery(charge_id):
        logger.warning("Premium delivery already claimed: charge_id=%s", charge_id)
        return payment_reconciliation_required("Premium delivery could not be claimed")
    ...
    try:
        ...
        return payment_reconciliation_required("Telegram Premium delivery failed")
    ...
    if not completed:
        ...
        return payment_reconciliation_required("Premium delivered but database finalization failed")
